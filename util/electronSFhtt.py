...
class ElectronSFhtt :
    ...
    def __init__( self, wp ):
        ...
        base = 'data/LeptonEfficiencies/Electron/'
        if wp == 'IdIso0p10' :
            self.elecSFFile = ROOT.TFile( base+'Electron_IdIso0p10_eff.root', 'r' )
        if wp == 'IdIso0p15' :
            self.elecSFFile = ROOT.TFile( base+'Electron_IdIso0p15_eff.root', 'r' )
        self.mcBarrel = self.elecSFFile.Get( 'ZMassEtaLt1p48_MC' )
        self.mcEndcap = self.elecSFFile.Get( 'ZMassEtaGt1p48_MC' )
        self.dataBarrel = self.elecSFFile.Get( 'ZMassEtaLt1p48_Data' )
        self.dataEndcap = self.elecSFFile.Get( 'ZMassEtaGt1p48_Data' )
