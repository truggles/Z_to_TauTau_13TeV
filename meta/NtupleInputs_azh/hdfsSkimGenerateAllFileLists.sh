...
for CHANNEL in eeet eett eemt eeem emmt mmtt mmmt emmm eeee mmmm; do
    ...
    for SAMPLE in HZZ; do
        ls ${HZZandDY4}/${SAMPLE}125_*_${CHANNEL}.root > skimmed/${SAMPLE}125_${CHANNEL}.txt
    done
done

echo "Overwrite failed DYJets4 files from above for EETT and MMTT (no svFit)"
echo ""
ls ${HZZandDY4}/*DYJets4_*_eett.root > skimmed/DYJets4_eett.txt
ls ${HZZandDY4}/*DYJets4_*_mmtt.root > skimmed/DYJets4_mmtt.txt
