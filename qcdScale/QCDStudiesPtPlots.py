...
def addDetails( hist ) :
    hist.SetStats(0)
    hist.GetXaxis().SetTitle("Leading #tau_{h} p_{T}")
    hist.GetYaxis().SetTitle("Secondary #tau_{h} p_{T}")


#xBin = array('d', [0,1,3,4.5,7,10])
xBin = array('d', [45,55,65,75,100,150])
...
def getHistos( decay="", dStr="" ) :
    ...
    for cut in cutMap.keys() :
        ...
        Td.Draw("pt_1:pt_2>>hData%s" % cut, dataCut)
        ...
        Tmc.Draw("pt_1:pt_2>>hMC%s" % cut, mcCut)
        ...
        Tqcd.Draw("pt_1:pt_2>>hQCDmc%s" % cut, mcCut)
        ...
        c2.SaveAs("/afs/cern.ch/user/t/truggles/www/qcdScale/qcdPtPlots/quad_%s%s.png" % (cut, decay) )
        ...
        c3.SaveAs("/afs/cern.ch/user/t/truggles/www/qcdScale/qcdPtPlots/ratio_%s%s.png" % (cut, decay) )
        ...
        p5.cd()
        histos[cut + "hData"].Draw("colz text")
        c5.SaveAs("/afs/cern.ch/user/t/truggles/www/qcdScale/qcdPtPlots/occupancy_%s%s.png" % (cut, decay) )
    ...
    c4.SaveAs("/afs/cern.ch/user/t/truggles/www/qcdScale/qcdPtPlots/OSvsSS-ddQCD%s.png" % decay)
