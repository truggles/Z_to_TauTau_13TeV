...
def renameBranches( sample, channel ) :
    ...
    oldFileName = '%s1BaseCut/%s.root' % (pre_, sample)
    newFileName = '%s2IsoOrderAndDups/%s_%s.root' % (pre_, sample, channel)
    dirName = channel
    treeName = 'Ntuple'
...
#Samples25ns = ['TT', 'WW']#, 'WW2l2n', 'WW4q', 'WW1l1n2q', 'WZJets', 'WZ1l1n2q', 'ZZ', 'ZZ4l', 'QCD15-20', 'QCD20-30', 'QCD30-50', 'QCD50-80', 'QCD80-120', 'QCD120-170', 'QCD170-300', 'QCD300-Inf']
...
if pre_ == '25ns' :
    ...
    for sample in samples :
        if sample != 'TT' :
            print sample
            renameBranches( sample, 'tt' )
            renameBranches( sample, 'em' )
        if sample == 'TT' :
            for i in range(0, 8) :
                TTname = '%s_%i' % (sample, i)
                print TTname
                renameBranches( TTname, 'tt' )
                renameBranches( TTname, 'em' )
                
else : print "Samples != 25ns"
